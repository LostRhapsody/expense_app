...
export default defineEventHandler(async (event) => {
  ...
  if (!checkValidKey(body.key)) {
    ...
    // check if key includes "envelopeBudget", if not this is a bad call
    if (!body.key.includes("envelopeBudget")) {
      response = setApiError(
        "The key did not include a 'envelopeBudget' indicator, could not set record."
      );
    } else {
